...
def _enable_2fa(api, email: str, password: str) -> str:
    """Helper: fully enable 2FA for a user. Returns the TOTP secret."""
    ...
    setup = api.client.post("/api/auth/2fa/setup", headers=headers).json()
    secret = setup["secret"]
...
class TestTwoFA:
    def test_setup_returns_secret_and_uri(self, api):
        resp = api.client.post(
            "/api/auth/2fa/setup",
            headers=auth(api.provider_token),
        )
    ...
    def test_enable_requires_valid_code(self, api):
        api.client.post(
            "/api/auth/2fa/setup",
            headers=auth(api.provider_token),
        )
    ...
    def test_enable_succeeds_with_valid_code(self, api):
        resp = api.client.post(
            "/api/auth/2fa/setup",
            headers=auth(api.provider_token),
        )
        ...
        assert resp.status_code == 200
        body = resp.json()
        assert body["message"] == "Two-factor authentication enabled"
        assert len(body["recovery_codes"]) == 8

    def test_enable_rejects_duplicate(self, api):
        _enable_2fa(api, "provider@example.com", "password")
        resp = api.client.post(
            "/api/auth/2fa/setup",
            headers=auth(api.provider_token),
        )
    ...
    def test_status_reports_enabled(self, api):
        resp = api.client.get(
            "/api/auth/2fa/status",
            headers=auth(api.provider_token),
        )
    ...
    def test_disable_requires_valid_code(self, api):
        _enable_2fa(api, "provider@example.com", "password")
        resp = api.client.post(
    ...
    def test_disable_succeeds_with_valid_code(self, api):
        secret = _enable_2fa(api, "provider@example.com", "password")
        totp = pyotp.TOTP(secret)
    ...
    def test_recovery_codes_returns_count(self, api):
        _enable_2fa(api, "provider@example.com", "password")
        resp = api.client.get(
    ...
    def test_regenerate_recovery_codes(self, api):
        secret = _enable_2fa(api, "provider@example.com", "password")
        totp = pyotp.TOTP(secret)
...
class TestTOTPResetPassword:
    def test_reset_requires_2fa(self, api):
        resp = api.client.post(
    ...
    def test_reset_rejects_invalid_totp(self, api):
        _enable_2fa(api, "provider@example.com", "password")
        resp = api.client.post(
            "/api/auth/reset-password",
            json={
    ...
    def test_reset_succeeds_with_valid_totp(self, api):
        secret = _enable_2fa(api, "provider@example.com", "password")
        totp = pyotp.TOTP(secret)
    ...
    def test_reset_rejects_old_password(self, api):
        secret = _enable_2fa(api, "provider@example.com", "password")
        totp = pyotp.TOTP(secret)
    ...
    def test_reset_succeeds_with_recovery_code(self, api):
        resp = api.client.post(
            "/api/auth/2fa/setup",
            headers=auth(api.provider_token),
        )
        ...
        enable_resp = api.client.post(
            "/api/auth/2fa/enable",
            headers=auth(api.provider_token),
            json={"code": totp.now()},
        )
        recovery_code = enable_resp.json()["recovery_codes"][0]
    ...
    def test_reset_rejects_used_recovery_code(self, api):
        resp = api.client.post(
            "/api/auth/2fa/setup",
            headers=auth(api.provider_token),
        )
    ...
    def test_reset_rejects_invalid_email(self, api):
        resp = api.client.post(
            "/api/auth/reset-password",
            json={
