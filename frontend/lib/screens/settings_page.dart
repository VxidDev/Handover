...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _load2FAStatus() async {
    try {
      final res = await Api.get('/api/auth/2fa/status');
      if (!mounted) return;
      setState(() => _twoFactorEnabled = res['enabled'] as bool);
    } catch (_) {}
  }
  ...
  Future<void> _disable2FA() async {
    ...
    try {
      await Api.post('/api/auth/2fa/disable', body: {'code': code});
      if (!mounted) return;
      setState(() => _twoFactorEnabled = false);
      _snack('Two-factor authentication disabled', isError: false);
    } catch (e) {
      if (!mounted) return;
      _snack(describeError(e));
    }
          ...
          children: [
            _SettingsTile(
              ...
              subtitle: _twoFactorEnabled
                  ...
                  : 'Not enabled — recommended for password reset',
              trailing: Switch(
